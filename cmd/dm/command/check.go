...
func newCheckCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err := prepare.CheckClusterPortConflict("nonexist-dummy-dm-cluster", &topo); err != nil {
				return err
			}
			if err := prepare.CheckClusterDirConflict("nonexist-dummy-dm-cluster", &topo); err != nil {
				return err
			}
