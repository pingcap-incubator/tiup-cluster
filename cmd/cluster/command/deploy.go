...
func fixDir(topo *meta.ClusterSpecification) func(string) string {
...
func checkClusterDirConflict(clusterName string, topo *meta.ClusterSpecification) error {
...
func checkClusterPortConflict(clusterName string, topo *meta.ClusterSpecification) error {
	clusterDir := meta.ProfilePath(meta.TiOpsClusterDir)
	fileInfos, err := ioutil.ReadDir(clusterDir)
	if err != nil && !os.IsNotExist(err) {
		return err
...
func buildDownloadCompTasks(version string, topo *meta.ClusterSpecification) []*task.StepDisplay {
